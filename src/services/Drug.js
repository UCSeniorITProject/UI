...
export async function getDrugWithFilter(filter) {
  const drugs = await Axios.get(
    `${process.env.REACT_APP_API_URL}/api/pharmacy-service/drug`,
    { params: filter }
  );
...
export async function updateDrug(drugId, drugInfo) {
  const drug = await Axios.patch(
    `${process.env.REACT_APP_API_URL}/api/pharmacy-service/drug/${drugId}`,
    { drug: drugInfo }
  );
  return drug.data.drug;
}
