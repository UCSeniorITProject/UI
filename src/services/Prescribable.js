...
export async function getPrescribableBreakdown(doctorId){
	const prescribableBreakdown = await Axios.get(`${process.env.REACT_APP_API_URL}/api/pharmacy-service/prescribable/doctor/${doctorId}/breakdown`);
	return prescribableBreakdown.data.data;
}

export async function getPrescribableBreakdownByPatientForDoctor(doctorId){
	const prescribableBreakdown = await Axios.get(`${process.env.REACT_APP_API_URL}/api/pharmacy-service/prescribable/doctor/${doctorId}/patient/breakdown`);
	return prescribableBreakdown.data.data;
}

export async function getNumPrescribablesPerMonthForDoctor(doctorId){
	const prescribableBreakdown = await Axios.get(`${process.env.REACT_APP_API_URL}/api/pharmacy-service/prescribable/doctor/${doctorId}/breakdown/by-month`);
	return prescribableBreakdown.data.data;
}
