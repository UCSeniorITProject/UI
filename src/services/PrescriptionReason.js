...
export async function getPrescribablesByReason(patientId){
	const prescribableByReasons = await Axios.get(`${process.env.REACT_APP_API_URL}/api/pharmacy-service/prescription-reason/${patientId}/by-prescribable`);
...
export async function getPrescriptionReasonCount(patientId){
	const prescriptionReasonCount = await Axios.get(`${process.env.REACT_APP_API_URL}/api/pharmacy-service/prescription-reason/${patientId}/by-patient`);
