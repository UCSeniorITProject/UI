...
class AddDrug extends React.Component {
  ...
  async componentDidMount() {
    try {
      const [drugTypes, drugs] = await Promise.all([
      ...
      this.setState({
        ...
        parentDrugs: drugs.map((x) => {
          return { id: x.drugId, label: x.name };
        }),
      });
    } catch (err) {
      var options = {};
  ...
  async createDrug() {
    try {
      const drug = await createDrug({
        ...
        federalDrugIdentifier: Number.parseInt(
          this.state.federalDrugIdentifier
        ),
        nonGenericParentId:
          this.state.nonGenericParentId === null
            ? null
            : this.state.nonGenericParentId,
  ...
  showPickDrugTypeMessage() {
    ...
    options = {
      ...
      message: (
        <div>
          <div>You must select a drug type to continue!</div>
        </div>
      ),
      type: "warning",
      icon: "tim-icons icon-bell-55",
                    ...
                    <Col className="pr-md-1" md="6">
                      ...
                      >
                        ...
                        <Input
                          ...
                          type="text"
                          onChange={(e) =>
                            this.change(e, "manufacturer", "length", 1)
                          }
                          onFocus={(e) =>
                    ...
                    <Col className="pr-md-1" md="6">
                      <InputGroup
                        className={classnames(
                          this.state.federalDrugIdentifierState,
                          {
                            "input-group-focus": this.state
                              .federalDrugIdentifierFocus,
                          }
                        )}
                      >
                        <InputGroupAddon addonType="prepend">
                        ...
                        <Input
                          ...
                          type="text"
                          onChange={(e) =>
                            this.change(e, "federalDrugIdentifier", "length", 1)
                          }
                          onFocus={(e) =>
                            this.setState({ federalDrugIdentifierFocus: true })
                          }
                          onBlur={(e) => {
                            this.setState({
                              federalDrugIdentifierFocus: false,
                            });
                            this.change(
                              e,
                              "federalDrugIdentifier",
