...
import {
  createPrescribable,
  getPrescribableWithFilter,
} from "../../services/Prescribable";
...
class DrugProfile extends React.Component {
  ...
  isPrescribableFormValid() {
    console.log(
      Object.entries(this.state.prescribable).filter(
        (x) =>
  ...
  async addPrescribable() {
    try {
                ...
                <Button
                  ...
                  onClick={(e) =>
                    this.props.history.push(
                      `/admin/prescribable/profile/${prescribableSaved.prescribableId}/`
                    )
                  }
                >
                  EDIT
      ...
      options = {
        ...
        message: (
          <div>
            <div>Succesfully created the prescribable!</div>
          </div>
        ),
        type: "success",
        icon: "tim-icons icon-bell-55",
  ...
  async componentDidMount() {
    ...
    try {
      ...
      const [
        ...
        getDrugWithFilter({ nonGenericParentId: 0, active: "Y" }),
        getDrugWithFilter({ drugId: drug[0].nonGenericParentId }),
        getPrescribableWithFilter({ active: "Y", drugId }),
        getDrugTypesWithFilter({}),
      ]);
      this.setState({
        drug: {
          ...this.state.drug,
      ...
      const presentDrugType = drugTypes.find(
        (x) => x.drugTypeId === drug[0].drugType
      );
      let drugProfileState = {
        ...
        parentDrugs: parentDrugs.map((x) => {
          return { value: x.drugId, label: x.name };
        }),
      ...
      this.setState({
        drug: drugProfileState,
        prescribables: prescribables.map((x) => {
          return {
            ...
            actions: (
              <div className="actions-right">
                <Button
                  ...
                  onClick={(e) =>
                    this.props.history.push(
                      `/admin/prescribable/profile/${x.prescribableId}/`
                    )
                  }
                >
                  EDIT
      ...
      case "length":
        if (this.verifyLength(event.target.value, stateNameEqualTo)) {
          this.setState(
            ...
            stateTree === "prescribable"
              ? this.setIsPrescribableFormValid
  ...
  isFormValid() {
    return (
      Object.entries(this.state.drug).filter(
        (x) =>
          x[0].includes("State") && x[1] !== null && x[1].includes("has-danger")
      ).length === 0 &&
      (!this.state.drug.isGeneric || this.state.drug.isGeneric) &&
      Number(this.state.drug.nonGenericParentId) !== 0
    );
  }
  ...
  setIsPrescribableFormValid() {
    this.setState({
      prescribable: {
        ...this.state.prescribable,
        isPrescribableFormValid: this.isPrescribableFormValid(),
      },
    });
  }
  ...
  async updateDrug() {
    ...
    try {
      ...
      options = {
        ...
        message: (
          <div>
            <div>Succesfully updated the drug!</div>
          </div>
        ),
        type: "success",
        icon: "tim-icons icon-bell-55",
                ...
                this.setState(
                  {
                    drug: {
                      ...this.state.drug,
                      nonGenericParentId: value.value,
                    },
                  },
                  () => {
                    this.setState({
                      drug: {
                        ...this.state.drug,
                        isDrugFormValid: this.isFormValid(),
                      },
                    });
                  }
                );
      ...
      parentDrugSelect = (
        <Col className="pr-md-1" md="6">
          ...
          <Switch
            onChange={async (e) => {
              this.setState(
                { drug: { ...this.state.drug, isGeneric: true } },
                () => {
                  this.setState({
                    drug: {
                      ...this.state.drug,
              ...
              <Card>
                ...
                <CardBody>
                  <Row>
                    <Col md="6">
                      <FormGroup
                        className={`has-label ${this.state.drug.nameState}`}
                      >
                        ...
                        <Input
                          ...
                          type="text"
                          onChange={(e) =>
                            this.change(e, "name", "length", "1", "drug")
                          }
                    ...
                    <Col md="6">
                      <FormGroup
                        className={`has-label ${this.state.drug.manufacturerState}`}
                      >
                    ...
                    <Col md="6">
                      <FormGroup
                        className={`has-label ${this.state.drug.federalDrugIdentifierState}`}
                      >
                        ...
                        />
                        {this.state.drug.federalDrugIdentifierState ===
                        "has-danger" ? (
                          <label className="error">
                ...
                <CardFooter>
                  ...
                  <Button
                    ...
                    color="primary"
                    type="submit"
                    disabled={!this.state.drug.isDrugFormValid}
                    onClick={(e) => this.updateDrug()}
                  >
                    Update Drug
                  </Button>
                </CardFooter>
              ...
              <Card>
                ...
                <CardBody>
                  <Row>
                    <Col md="3">
                      <FormGroup
                        className={`has-label ${this.state.prescribable.dosageState}`}
                      >
                        <label>Dosage</label>
                        <Input
                          name="dosage"
                        ...
                        />
                        {this.state.prescribable.dosageState ===
                        "has-danger" ? (
                          <label className="error">
                              ...
                              this.setState(
                                {
                                  prescribable: {
                                    ...this.state.prescribable,
                                    dosageUnit: value.value,
                                    dosageUnitState: "has-success",
                                    dosageUnitSelect: {
                                      value: value.value,
                                      label: value.label,
                                    },
                              ...
                              this.setState(
                                {
                                  prescribable: {
                                    ...this.state.prescribable,
                                    dosageFrequency: value.value,
                                    dosageFrequencySelect: {
                                      value: value.value,
                                      label: value.label,
                                    },
                                    dosageFrequencyState: "has-success",
                                  },
                                },
                                this.setIsPrescribableFormValid
                              );
                    ...
                    <Col md="3">
                      <FormGroup
                        className={`has-label ${this.state.prescribable.minWeightState}`}
                      >
                        ...
                        />
                        {this.state.prescribable.minWeightState ===
                        "has-danger" ? (
                          <label className="error">
                            Please enter a valid minimum weight as a number (in
                            lbs.)
                          </label>
                        ) : null}
                      </FormGroup>
                        ...
                        <FormGroup check>
                          <Label check>
                            <Input
                              type="checkbox"
                              checked={
                                (this.state.prescribable.requiredGender ===
                                  "M" ||
                                  this.state.prescribable.requiredGender ===
                                    "B") &&
                                this.state.prescribable.requiredGender !== null
                              }
                              onChange={(e) =>
                                this.setState(
                                  {
                                    prescribable: {
                                      ...this.state.prescribable,
                                      requiredGenderState: "has-success",
                                      requiredGender:
                                        this.state.prescribable
                                          .requiredGender === null
                                          ? "M"
                    ...
                    <Col md="4">
                      <FormGroup
                        className={`has-label ${this.state.prescribable.nameState}`}
                      >
                        <label>Name</label>
                        <Input
                          name="name"
                ...
                <CardFooter>
                  <Button
                    ...
                    color="primary"
                    type="submit"
                    disabled={!this.state.prescribable.isPrescribableFormValid}
                    onClick={(e) => this.addPrescribable()}
                  >
                    Add Prescribable
                  </Button>
                </CardFooter>
