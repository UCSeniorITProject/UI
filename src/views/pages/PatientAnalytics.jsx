...
import {
  getPrescribablesByReason,
  getPrescriptionReasonCount,
} from "../../services/PrescriptionReason";
import {
  getPrescriptionPrescribableDrugCountForLastYear,
  getCountOfPrescribablePerDoctor,
} from "../../services/PrescriptionPrescribableDrug";
class PatientAnalytics extends React.Component {
  ...
  async componentDidMount() {
    try {
      ...
      this.setState({
        prescriptionsByMonthData: this.createPrescriptionsByMonthChart(
          prescriptionsByMonth
        ),
        prescribablesByReason: this.createPrescribablesByReasonChart(
          prescribablesByReason
        ),
        prescriptionPrescribableDrugTotalForYear: this.createPrescriptionPrescribableDrugTotalForYear(
          prescriptionPrescribableDrugTotalForYear
        ),
        countOfPrescrtibablePerDoctor: this.getCountOfPrescribablePerDoctor(
          countOfPrescrtibablePerDoctor
        ),
        prescriptionReasonCount: this.getPrescriptionReasonCountChart(
          prescriptionReasonCount
        ),
        prescribableByMonths: this.getPrescribableByMonthChart(
          prescribableByMonth
        ),
      });
    } catch (err) {
  ...
  createPrescriptionPrescribableDrugTotalForYear(data) {
    const canvas = document.getElementById(
      "prescriptionPrescribableDrugTotalForYearChart"
    );
    let ctx = canvas.getContext("2d");
    var gradientStroke = ctx.createLinearGradient(0, 230, 0, 50);
  ...
  showErrorLoadingDataMessage() {
    ...
    options = {
      ...
      message: (
        <div>
          <div>
            An error occurred while loading patient data. Please try again later
