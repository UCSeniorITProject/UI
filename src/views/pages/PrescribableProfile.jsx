  ...
  isFormValid() {
    return (
      Object.entries(this.state).filter(
        (x) => x[0].includes("State") && x[1] !== null && x[1] === "has-danger"
      ).length === 0
    );
  }
  ...
  async componentDidMount() {
    ...
    try {
      ...
      this.setState({
        ...
        minWeightState: null,
        requiredGender:
          prescribable[0].requiredGender === "B" ||
          prescribable[0].requiredGender === ""
            ? "B"
            : prescribable[0].requiredGender,
  ...
  async updatePrescribable() {
    try {
      ...
      options = {
        ...
        message: (
          <div>
            <div>Succesfully updated the prescribable!</div>
          </div>
        ),
        type: "Success",
          ...
          <Row>
            <Col md="12">
              <Card>
                <CardHeader>
                  <CardTitle tag="h4">
                    Update Prescribable (Parent Drug:{" "}
                    {this.state.parentDrugName})
                  </CardTitle>
                </CardHeader>
                <CardBody>
                  <Row>
                    <Col md="3">
                      <FormGroup
                        className={`has-label ${this.state.dosageState}`}
                      >
                        <label>Dosage</label>
                        <Input
                          name="dosage"
                    ...
                    <Col md="3">
                      <FormGroup
                        className={`has-label ${this.state.minWeightState}`}
                      >
                        ...
                        <FormGroup check>
                          <Label check>
                            <Input
                              type="checkbox"
                              checked={
                                (this.state.requiredGender === "M" ||
                                  this.state.requiredGender === "B") &&
                                this.state.requiredGender !== null
                              }
                              onChange={(e) =>
                                this.setState(
                                  {
                ...
                <CardFooter>
                  <Button
                    ...
                    color="primary"
                    type="submit"
                    disabled={!this.state.isPrescribableFormValid}
                    onClick={(e) => this.updatePrescribable()}
                  >
                    Update Prescribable
                  </Button>
                </CardFooter>
