...
class Prescribe extends React.Component {
  ...
  showSuccessfullySavedMessage() {
    ...
    options = {
      ...
      message: (
        <div>
          <div>
            Succesfully created the prescription! Redirecting to the patient
            profile page...
          </div>
        </div>
      ),
  ...
  async finishButtonClick() {
    ...
    try {
      ...
      for (let i = 0; i < this.state.prescribableReasonsMapped.length; i++) {
        prescriptionPrescribableDrugsToCreate.push(
          createPrescriptionPrescribableDrug(
            {
              prescriptionId: prescription.prescriptionId,
              prescribableId: this.state.prescribableReasonsMapped[i]
                .prescribableId,
              patientId: this.state.patientId,
              active: "Y",
      ...
      const prescriptionPrescribableDrugs = await Promise.all(
        prescriptionPrescribableDrugsToCreate
      );
      ...
      this.state.prescribableReasonsMapped.forEach((x, index) => {
        x.reasons.forEach((y, index) => {
          prescriptionPrescribableDrugReasonsToCreate.push(
            createPrescriptionPrescribableDrugReason({
              prescriptionReasonId: "value" in y ? y.value : y,
              prescriptionPrescribableDrugId: prescriptionPrescribableDrugs.filter(
                (z) => z.prescribableId === x.prescribableId
              )[0].prescriptionPrescribableDrugId,
              active: "Y",
            })
          );
        });
      ...
      setTimeout(() => {
        this.props.history.push(
          `/admin/patient/profile/${this.state.patientId}`
        );
      }, 3000);
    } catch (err) {
  ...
  showAbleToBePrescribed() {
    ...
    options = {
      ...
      message: (
        <div>
          <div>Succesfully prescribed the drug! Redirecting...</div>
        </div>
      ),
      type: "success",
      icon: "tim-icons icon-bell-55",
  ...
  showNotAbleToBePrescribed() {
    ...
    options = {
      ...
      message: (
        <div>
          <div>
            This patient already has a prescription for this drug!
            Redirecting...
          </div>
        </div>
      ),
