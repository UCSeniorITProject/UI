...
class PickPatient extends React.Component {
  ...
  async componentDidMount() {
    ...
    const patientList = patients.map((x) => {
      return {
        patientId: x.patientId,
        firstName: x.firstName,
        lastName: x.lastName,
        birthDate: moment(x.dateOfBirth).format("MM/DD/YYYY"),
        ...
        actions: (
          <div className="actions-right">
            <Button
              ...
              onClick={(e) =>
                this.props.history.push(
                  `/admin/patient/profile/${x.patientId}/`
                )
              }
            >
            ...
            <Button
              className={className("btn-icon", "btn-link", "like", {
                "btn-neutral":
                  this.state.selectedPatientID === Number(x.patientId),
              })}
              size="sm"
              color={
                this.state.selectedPatientID === Number(x.patientId)
              ...
              onClick={(e) => {
                this.props.onChildStateChange("patientId", x.patientId);
                this.setState({
                  currentlySelectedPatientName: `${x.firstName} ${x.lastName}`,
                  selectedPatientID: Number(x.patientId),
                });
              }}
            >
  ...
  showPickPatientRequiredMessage() {
    ...
    options = {
      ...
      message: (
        <div>
          <div>You must select a patient to create the prescription for!</div>
        </div>
      ),
      type: "warning",
        ...
        <Row>
          <Col md="9">
            <h5 className="info-text float-left">
              <b>
                Patient Picker (Currently Selected Patient:{" "}
                {this.state.currentlySelectedPatientName})
              </b>
            </h5>
          </Col>
          <Col md="3">
