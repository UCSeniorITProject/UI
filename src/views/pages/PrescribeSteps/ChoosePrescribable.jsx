...
class ChoosePrescribable extends React.Component {
  ...
  showPickPrescribableMessage() {
    ...
    options = {
      ...
      message: (
        <div>
          <div>You must select a prescribable to continue!</div>
        </div>
      ),
      type: "warning",
  ...
  isValidated = () => {
    const isValid =
      this.state.prescribables.length !== 0 &&
      this.state.prescriptionStartDate !== null &&
      this.state.prescriptionEndDate > this.state.prescriptionStartDate;
    if (!isValid) {
            ...
            <Select
              ...
              onChange={(value) => {
                document.dispatchEvent(
                  new CustomEvent("prescribableChanged", { detail: value })
                );
                if (value.length === 0) {
                  this.showPickPrescribableMessage();
                }
                this.setState({
                  ...
                  selectedPrescribables: value.map((x) => x.value),
                });
