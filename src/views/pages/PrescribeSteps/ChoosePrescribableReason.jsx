...
class PickPrescribableReasons extends React.Component {
  ...
  async componentDidMount() {
    ...
    this.setState({
      ...
      prescribableReasons: reasons.map((x) => {
        return {
          label: `${x.reasonCode}: ${x.shortSummary}`,
          value: x.prescriptionReasonId,
        };
      }),
    });
  }
  ...
  showPickReason() {
    ...
    options = {
      ...
      message: (
        <div>
          <div>
            You must select a reason to assign to a prescribable for each
            prescribable!
          </div>
        </div>
      ),
  ...
  isValidated() {
    const isValid =
      this.state.prescribableReasonsMapped.filter((x) => x.reasons.length === 0)
        .length === 0 && this.state.prescribableReasonsMapped.length !== 0;
    if (!isValid) {
      this.showPickReason();
    } else {
      ...
      reasonSelect = (
        <Select
          ...
          onChange={(value) => {
            ...
            const currentlySelectedPrescribable = this.state
              .selectedPrescribable.value;
            let isPrescribableReasonFound = false;
            const prescribableReasonsMapped =
              this.state.prescribableReasonsMapped.length === 0
                ? [
                    {
                      prescribableId: currentlySelectedPrescribable,
                      reasons: value,
                    },
                  ]
                : this.state.prescribableReasonsMapped.map((x) => {
            ...
            if (
              !isPrescribableReasonFound &&
              prescribableReasonsMapped.filter(
                (x) => x.prescribableId === currentlySelectedPrescribable
              ).length === 0
            ) {
              prescribableReasonsMapped.push({
                prescribableId: currentlySelectedPrescribable,
                reasons: value,
              });
            }
            this.props.onChildStateChange(
              "prescribableReasonsMapped",
              prescribableReasonsMapped
            );
            this.setState({
          ...
          <Col md="6">
            <Select
              ...
              onChange={(value) => {
                ...
                const reasons = this.state.prescribableReasonsMapped.filter(
                  (x) => x.prescribableId === value.value
                );
                this.setState({
                  selectedPrescribable: value,
                  selectedReasons: reasons.length > 0 ? reasons[0].reasons : [],
                });
              }}
              options={[
                {
