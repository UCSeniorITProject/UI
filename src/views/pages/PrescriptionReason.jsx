...
import {
  createPrescriptionReason,
  getPrescriptionReasonWithFilter,
  updatePrescriptionReason,
} from "../../services/PrescriptionReason";
...
class PrescriptionReason extends React.Component {
  ...
  async addReason() {
    try {
      ...
      const savedPrescriptionReason = await createPrescriptionReason(
        prescriptionReason
      );
      ...
      this.setState({
        ...
        reasonCodeState: null,
        prescriptionReasons: [
          ...this.state.prescriptionReasons,
          {
            ...
            createdAt: moment(savedPrescriptionReason.createdAt).format(
              "MM/DD/YYYY"
            ),
            actions: (
              <div className="actions-right">
                <Button
                  ...
                  className="btn-fill"
                  onClick={(e) =>
                    this.updatePrescriptionReasonModal(
                      savedPrescriptionReason.prescriptionReasonId
                    )
                  }
                >
                  Update
      ...
      this.showSuccessfulResponseMessage(
        "Succesfully created the prescription reason"
      );
    } catch (err) {
  ...
  async componentDidMount() {
    try {
      const prescriptionReasons = await getPrescriptionReasonWithFilter({
        active: "Y",
      });
          ...
          actions: (
            <div className="actions-right">
              <Button
                ...
                className="btn-fill"
                onClick={(e) =>
                  this.updatePrescriptionReasonModal(x.prescriptionReasonId)
                }
              >
                Update
              </Button>
  ...
  async updatePrescriptionReason() {
    try {
      await updatePrescriptionReason(
        this.state.currentlySelectedPrescriptionReasonId,
        {
          shortSummary: this.state
            .currentlySelectedPrescriptionReasonShortSummary,
          longSummary: this.state
            .currentlySelectedPrescriptionReasonLongSummary,
      ...
      this.setState({
        prescriptionReasons: this.state.prescriptionReasons.map((x) => {
          if (
            x.prescriptionReasonId ===
            this.state.currentlySelectedPrescriptionReasonId
          ) {
            return {
              prescriptionReasonId: this.state
                .currentlySelectedPrescriptionReasonId,
              shortSummary: this.state
                .currentlySelectedPrescriptionReasonShortSummary,
              createdAt: x.createdAt,
              reasonCode: this.state
                .currentlySelectedPrescriptionReasonReasonCode,
              actions: (
                <div className="actions-right">
                  <Button
      ...
      this.showSuccessfulResponseMessage(
        "Succesfully updated the prescription reason!"
      );
    } catch (err) {
      this.showInternalServerErrorMessage();
    }
  ...
  async updatePrescriptionReasonModal(currentlySelectedPrescriptionReasonId) {
    try {
      const prescriptionReason = await getPrescriptionReasonWithFilter({
        prescriptionReasonId: currentlySelectedPrescriptionReasonId,
      });
      this.setState({
        currentlySelectedPrescriptionReasonId:
          prescriptionReason[0].prescriptionReasonId,
        currentlySelectedPrescriptionReasonLongSummary:
          prescriptionReason[0].longSummary,
        currentlySelectedPrescriptionReasonReasonCode:
          prescriptionReason[0].reasonCode,
        currentlySelectedPrescriptionReasonShortSummary:
          prescriptionReason[0].shortSummary,
      });
    } catch (err) {
      this.showInternalServerErrorMessage();
    }
  ...
  isAddFormValid() {
    return (
      Object.entries(this.state).filter(
        (x) =>
          x[0].includes("State") &&
          !x[0].includes("currentlySelected") &&
          (x[1] === null || x[1] === "has-danger")
      ).length === 0
    );
  }
      ...
      case "length":
        if (this.verifyLength(event.target.value, stateNameEqualTo)) {
          this.setState(
            ...
            stateTree === "add"
              ? this.setIsAddFormValid
              : this.setIsUpdateFormValid
          );
        } else {
          this.setState(
            {
                ...
                <Card>
                  ...
                  <CardBody>
                    <Row>
                      <Col md="6">
                        <FormGroup
                          className={`has-label ${this.state.reasonCodeState}`}
                        >
                          ...
                          <Input
                            ...
                            type="text"
                            onChange={(e) =>
                              this.change(e, "reasonCode", "length", "1", "add")
                            }
                      ...
                      <Col md="6">
                        <FormGroup
                          className={`has-label ${this.state.shortSummaryState}`}
                        >
                      ...
                      <Col md="12">
                        <FormGroup
                          className={`has-label ${this.state.longSummaryState}`}
                        >
                  ...
                  <CardFooter>
                    <Button
                      ...
                      color="primary"
                      type="submit"
                      disabled={!this.state.isAddFormValid}
                      onClick={(e) => this.addReason()}
                    >
                      Add Reason
                    </Button>
                  </CardFooter>
                ...
                <Card>
                  ...
                  <CardBody>
                    <Row>
                      <Col md="6">
                        <FormGroup
                          className={`has-label ${this.state.currentlySelectedPrescriptionReasonReasonCodeState}`}
                        >
                          <label>Reason Code</label>
                          <Input
                            name="reasoncode"
                      ...
                      <Col md="6">
                        <FormGroup
                          className={`has-label ${this.state.currentlySelectedPrescriptionReasonShortSummaryState}`}
                        >
                          <label>Short Summary</label>
                          <Input
                            name="shortsummary"
                      ...
                      <Col md="12">
                        <FormGroup
                          className={`has-label ${this.state.currentlySelectedPrescriptionReasonLongSummaryState}`}
                        >
                          <label>Long Summary</label>
                          <Input
                            name="longsummary"
                  ...
                  <CardFooter>
                    <Button
                      ...
                      type="submit"
                      disabled={
                        !this.state.isUpdateFormValid ||
                        this.state.currentlySelectedPrescriptionReasonId ===
                          null
                      }
                      onClick={(e) => this.updatePrescriptionReason()}
                    >
                      Update Reason
                    </Button>
                  </CardFooter>
